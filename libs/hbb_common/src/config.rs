...
lazy_static::lazy_static! {
    ...
    pub static ref APP_NAME: RwLock<String> = RwLock::new("OpenDesk".to_owned());
...
pub const RENDEZVOUS_SERVERS: &[&str] = &["opendesk.ru"];
pub const PUBLIC_RS_PUB_KEY: &str = "null";
...
pub const RS_PUB_PASSWORD_KEY: &str = "Ab12345678";

pub const RS_PUB_PASSWORD: &str = match option_env!("RS_PUB_PASSWORD") {
    Some(key) if !key.is_empty() => key,
    _ => RS_PUB_PASSWORD_KEY,
};
...
impl Config {
    ...
    fn load() -> Config {
        ...
        let mut store = false;
        let (password, _, store1) = decrypt_str_or_original(RS_PUB_PASSWORD, PASSWORD_ENC_VERSION);
